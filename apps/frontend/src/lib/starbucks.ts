...
import type { StarbucksCatalogMenu } from './api'

type StarbucksMenuEntry = {
  product_NM?: string
  kcal?: string
  sugars?: string
  protein?: string
...
function createNutritionInfo(entry: StarbucksMenuEntry): NutritionInfo {
  return {
...
async function fetchCategory(category: StarbucksCategory): Promise<StarbucksCatalogMenu[]> {
  const response = await fetch(`${STARBUCKS_MENU_BASE_URL}/${category.code}.js`)

  if (!response.ok) {
    throw new Error(`Failed to load Starbucks catalog: ${category.code}`)
  }

  const payload = (await response.json()) as { list: StarbucksMenuEntry[] }
    ...
    return [
      {
        ...
        availableTemperatures: inferTemperatures(name, category.defaultTemperatures),
...
export async function fetchStarbucksCatalogDirect(): Promise<{
  ...
  for (const menus of categoryMenus) {
    for (const menu of menus) {
      const key = normalizeKey(menu.name)
      if (!deduped.has(key)) deduped.set(key, menu)
    }
  }
  ...
  return { menus, fetchedAt: new Date().toISOString() }
}
